...
@RestController
@RequestMapping(
    "/applications/{application}/clusters/{account}/{clusterName}/tencent/serverGroups/{serverGroupName}")
public class TencentServerGroupController {
  ...
  public ResponseEntity getScalingActivities(
    ...
    TencentServerGroup serverGroup =
        tencentClusterProvider.getServerGroup(account, region, serverGroupName, false);
    String autoScalingGroupId = (String) serverGroup.getAsg().get("autoScalingGroupId");
    AutoScalingClient client =
        new AutoScalingClient(
            ((TencentNamedAccountCredentials) credentials).getCredentials().getSecretId(),
